...
const WORKER_VERSION = "2026-07-13-hard-narration-clock-v17";
...
const runScript = async (page, script, nova, narrationMap, timelineBaseMs = Date.now(), recordingBudgetMs = 0) => {
    ...
    try {
      ...
      if (step.action === "goto") {
        ...
        if (page.url() === url) {
          ...
          await page.goto(url, { waitUntil: "domcontentloaded", timeout: Math.max(1000, Math.min(30000, remainingBudgetMs() || 30000)) });
...
const processFlow = async ({ flow, nova }) => {
  ...
  const narrationClockMs = applyContinuousNarrationTimeline(narrationMap);
  const narrationBudgetMs = narrationClockMs > 0 ? narrationClockMs + NARRATION_TAIL_MS : 0;
  console.log(`[narrate] preloaded ${narrationMap.length} segments`);
  console.log(`[timing] narration_clock=${narrationClockMs}ms recording_budget=${narrationBudgetMs}ms`);
  ...
  try {
    stepReport = await runScript(page, flowScript, nova, narrationMap, scriptStartedAt, narrationBudgetMs);
    if (await detectLanguageGate(page)) {
  ...
  // v16: narration is the authoritative clock. When narration exists, cap the
  // final video to narration length + a short tail so the audio can breathe.
  // The screen recording is trimmed (or padded) to match. Prevents the video
  // from stretching to 2-3 minutes when pages take a while to catch up after
  ...
  if (targetVideoMs > 0) {
    ...
    if (padMs > 80) {
      ...
      stage("trim-recording-to-narration", `${(-padMs / 1000).toFixed(3)}s cut`);
    }
