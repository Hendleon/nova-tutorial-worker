...
const processFlow = async ({ flow, nova }) => {
  ...
  let stepReport = { skipped: [], ran: 0, total: (flow.script || []).length };
  try {
    stepReport = await runScript(page, flow.script, nova, narrationMap);
    if (await detectLanguageGate(page)) {
  ...
  return { composited_url: viewUrl, recording_url: null, captions_srt_url: captionsSrtUrl, captions_vtt_url: captionsVttUrl, step_report: stepReport };
};
...
const loop = async () => {
  while (true) {
    try {
      ...
      try {
        const { composited_url, recording_url, captions_srt_url, captions_vtt_url, step_report } = await processFlow({ flow, nova });
        await api({ action: "complete", id: flow.id, composited_url, recording_url, captions_srt_url, captions_vtt_url, step_report });
